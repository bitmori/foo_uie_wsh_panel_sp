...
namespace helpers
{
	BOOL CALLBACK CFindWnd::FindChildClassHwnd(HWND hwnd, LPARAM lParam) {
		CFindWnd *pfw = (CFindWnd*)lParam;
		if(hwnd){
			::GetClassName(hwnd,pfw->m_name,255);
			if(_tcscmp(pfw->m_name,pfw->m_classname)==0){
				pfw->m_hWnd = hwnd;
				(pfw->m_count)--;
			}
		}
		if(pfw->m_count<=0)return FALSE;
		return TRUE;// keep looking
	}

	CFindWnd::CFindWnd(HWND hwndParent, LPCTSTR classname, int count)
	: m_hWnd(NULL), m_classname(classname),m_count(count)
	{
		::EnumChildWindows(hwndParent, FindChildClassHwnd, (LPARAM)this);
	}

	int TTCode(std::string& ttHexStr, int lrcId){
		int length = ttHexStr.length()>>1;
		int* song = new int[length];
		char* endPtr;
		for (int i = 0; i < length; i++) {
			song[i] = (int)strtol(ttHexStr.substr(i * 2, 2).c_str(), &endPtr, 16);
		}
		int t1 = 0, t2 = 0, t3 = 0;
		...
		int t5 = (int)TTConv(t2 ^ t3);
		t5 = (int)TTConv(t5 + (t1 | lrcId));
		t5 = (int)TTConv(t5 * (t1 | t3));
		t5 = (int)TTConv(t5 * (t2 ^ lrcId));
