...
LRESULT CALLBACK CInputBox::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) 
	{
		case WM_CREATE:
            ...
            lstrcpy(lfont.lfFaceName, _T("Segoe UI"));
            lfont.lfHeight = 16;
            ...
            // button OK
			m_hWndOK = CreateWindowEx(WS_EX_CONTROLPARENT,
				_T("button"),_T("OK"),
            ...
            // button Cancel
			m_hWndCancel = CreateWindowEx(WS_EX_CONTROLPARENT,
				_T("button"),_T("Cancel"),
				WS_VISIBLE | WS_CHILD | WS_TABSTOP, 
            ...
            // static Propmpt
			m_hWndPrompt = CreateWindowEx(WS_EX_CONTROLPARENT,
				_T("static"),_T(""),
				WS_VISIBLE | WS_CHILD, 
...
BOOL CInputBox::DoModal(LPCTSTR szCaption, LPCTSTR szPrompt, LPCTSTR szDefText)
{
	RECT r;
	GetWindowRect(GetDesktopWindow(), &r);
    ...
    SetWindowText(m_hWndPrompt, szPrompt);
	SetWindowText(m_hWndEdit, szDefText);
