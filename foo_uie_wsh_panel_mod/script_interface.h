...
__interface IThemeManager: IDisposable
{
	...
	STDMETHOD(DrawThemeTextEx)(IGdiGraphics * gr, BSTR text, VARIANT color, int x, int y, int w, int h, [defaultvalue(10)]int iGlowSize, [defaultvalue(0)] int format);
};
...
__interface IWSHUtils: IDispatch
{
	...
	//STDMETHOD(ShareInfo)(UINT target, BSTR name, VARIANT info);
};
...
[
	...
	pointer_default(unique),
	library_block,
	uuid("24282840-DED5-4A67-8204-92388577B0DC")
]
__interface IWSHAliens: IDispatch
{
	[propget] STDMETHOD(fbHWND)([out,retval] UINT * p);
	[propget] STDMETHOD(IsWin6)([out,retval] VARIANT_BOOL * p);
	STDMETHOD(GetHWND)(BSTR classID, [out,retval] UINT * p);
	STDMETHOD(GetHWND2)(BSTR title, [out,retval] UINT * p);
	STDMETHOD(GetChildHWND)(UINT parentHWND, BSTR classID, int index, [out,retval] UINT * p);
	STDMETHOD(GetParentHWND)(UINT hWnd, [out,retval] UINT * p);

	STDMETHOD(SendMessageEx)(UINT hWnd, UINT MsgNum, UINT wParam, UINT lParam);
	STDMETHOD(ReleaseCaptureEx)();
	STDMETHOD(ShareInfo)(UINT target, VARIANT name, VARIANT info);

	STDMETHOD(MoveWindowEx)(UINT hWnd, int x, int y, int w, int h, [defaultvalue(-1)] VARIANT_BOOL refresh);
	STDMETHOD(GetWindowInfoEx)(UINT hWnd, int Idx ,[out,retval] int * p);
	STDMETHOD(GetWindowInfoAx)(UINT hWnd, [out,retval] VARIANT * p);
	
	STDMETHOD(ShowWindowEx)(UINT hWnd, int command);
	STDMETHOD(IsWindowVisibleEx)(UINT hWnd, [out,retval] VARIANT_BOOL * p);
	STDMETHOD(IsMinimizedEx)(UINT hWnd, [out,retval] VARIANT_BOOL * p);
	STDMETHOD(IsMaximizedEx)(UINT hWnd, [out,retval] VARIANT_BOOL * p);

	STDMETHOD(CreateGlass)(UINT hWnd, int l, int r, int t, int b);
	STDMETHOD(IsGlassEnabled)([out,retval] VARIANT_BOOL * p);
	
	STDMETHOD(ShowScrollBarEx)(UINT hWnd, int target, VARIANT_BOOL show);
	STDMETHOD(IsScrollbarVisible)(UINT hWnd, int target, [out,retval] VARIANT_BOOL * p);
	
	STDMETHOD(CreateCursor)(BSTR filePath, [out,retval] LONG * p);
	STDMETHOD(ChangeCursor)(LONG hCUR,[out,retval] LONG * p);
	STDMETHOD(ReleaseCursor)(LONG hCUR);

	STDMETHOD(AddStyle)(UINT hWnd, long style);
	STDMETHOD(RemoveStyle)(UINT hWnd, long style);
	STDMETHOD(AddExStyle)(UINT hWnd, long exstyle);
	STDMETHOD(RemoveExStyle)(UINT hWnd, long exstyle);

	STDMETHOD(GetWindowStyleEx)(UINT hWnd, [out,retval] UINT* p);
	STDMETHOD(SetWindowStyleEx)(UINT hWnd, int style);

//	STDMETHOD(DecodeBase64Image)(BSTR data,[out,retval] IGdiBitmap **pp);

	STDMETHOD(CalcTTCode)(int ID, BSTR HexString,[out,retval] int * p);
//	STDMETHOD(HttpRequestText)(BSTR url, [out,retval] BSTR * pp);

	STDMETHOD(CreateMessageBox)(UINT hWnd, BSTR caption, BSTR prompt, UINT type, [out,retval] int *p);
	STDMETHOD(CreateInputBox)(UINT hWnd,BSTR caption, BSTR prompt,BSTR defText,[out,retval] BSTR * pp);
	STDMETHOD(CreateFileDialog)(UINT hWnd, BSTR fileType, BSTR ext,BSTR window_title, VARIANT_BOOL save, [out,retval] BSTR * pp);
	
//	STDMETHOD(RunMainMenuCommandByGUID)(BSTR bguid, [out,retval] VARIANT_BOOL * p);
//	STDMETHOD(RunContextMenuCommandByGUID)(BSTR bguid,UINT flag, [out,retval] VARIANT_BOOL * p);
//	STDMETHOD(RunContextMenuCommandByGUIDWithMetadb)(BSTR bguid, VARIANT handle, UINT flag,[out,retval] VARIANT_BOOL * p);
//	STDMETHOD(GetCommandGUID)(BSTR name,[out,retval] BSTR * pp);

	STDMETHOD(AnimateWindowEx)(UINT hWnd,UINT time, UINT flags);
};
_COM_SMARTPTR_TYPEDEF(IWSHAliens, __uuidof(IWSHAliens));