...
STDMETHODIMP ThemeManager::DrawThemeTextEx(IGdiGraphics * gr, BSTR text, VARIANT color, int x, int y, int w, int h,int iGlowSize, int format)
{
	TRACK_FUNCTION();
	...
	//part_id = TEXT_LABEL(8), state_id = 0
	...
	if (hBmp == NULL) return S_OK;
	HGDIOBJ hBmpOld = SelectObject(hMemDC, hBmp);

	DTTOPTS dttopts = {0};  
	dttopts.dwSize = sizeof(DTTOPTS);
	dttopts.dwFlags = DTT_TEXTCOLOR | DTT_GLOWSIZE | DTT_COMPOSITED;
	dttopts.iGlowSize = iGlowSize;  //size of glow area
	dttopts.crText = ExtractColorFromVariant(color);
	
	RECT rcText = {0, 0, w, h};

	HRESULT hr = ::DrawThemeTextEx(m_theme, hMemDC,  m_partid, m_stateid, text, -1, format, &rcText, &dttopts);
